...
import { dispatchUpdateProblem, dispatchDeleteProblem } from '../../../../redux/actions/problemAction';
...
const Problem = () => {
  ...
  const [problem, setProblem] = useState({ name: "", description: "", id: "", tag_one: "", tag_two: "", submitted: "" });
  ...
  const handleClick = (pname, des, id, tech_one, tech_two) => {
    setProblem({ name: pname, description: des, id: id, tag_one: tech_one, tag_two: tech_two });
    setDialog(!dialog)
  }
  const closeBox = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    e.preventDefault();
    if (isEmpty(problem.name && problem.description && problem.id && problem.tag_one && problem.tag_two)) {
      toast.error('All fields should be filled !', { theme: "colored" });
      return;
    }
    try {
      var res = await axios.put(`/api/problem/${problem.id}`, { name: problem.name, description: problem.description, tag_one: problem.tag_one, tag_two: problem.tag_two, submitted: problem.submitted },
        {
  ...
  const handleDelete = async () => {
    try {
      var res = await axios.post(`/api/problem/${problem.id}`,{},
      {
        headers: { Authorization: token },
      }
      );
  ...
  const handleFinalSubmit = async (e) => {
    ...
    try {
      var res = await axios.post(`/api/problem/submit/${problem.id}`,{},
      {
        headers: { Authorization: token },
      }
      );
        ...
        <div className="grid_card">
          ...
          <div style={{ height: '50px', display: 'flex', justifyContent: 'space-between', borderTop: '2px solid black', padding: '10px' }}>
            ...
            <div style={{ display: 'flex' }}>
              <div className="edit_icon" onClick={() => handleClick(problemName, item.description, id, tech_one, tech_two)} style={{ marginRight: "20px" }}>
