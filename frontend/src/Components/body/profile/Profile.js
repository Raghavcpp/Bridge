...
const Profile = () => {
  ...
  const changeAvatar = async (e) => {
    ...
    try {
      ...
      if (!file)
        return setData({ ...data, err: "No file uploaded", success: "" });
      if (file.size > 1024 * 1024)
        return setData({ ...data, err: "file is too large", success: "" });
      if (file.type !== "image/jpeg" && file.type !== "image/png")
        return setData({ ...data, err: "invalid format", success: "" });
      let formData = new FormData();
      formData.append("file", file);
      setLoading(true);
      const res = await API.post("/api/upload_avatar", formData, {
        headers: {
